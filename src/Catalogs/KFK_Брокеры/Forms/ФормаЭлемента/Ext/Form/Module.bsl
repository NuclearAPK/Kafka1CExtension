...
&НаКлиенте
Асинх Процедура СохранитьНастройкиВФайл(Команда)
	
	СписокНастроек = НастройкиБрокера();
	
	Режим = РежимДиалогаВыбораФайла.Сохранение;
	ДиалогСохраненияФайла = Новый ДиалогВыбораФайла(Режим);
	ДиалогСохраненияФайла.Расширение = "conf";
	ДиалогСохраненияФайла.Фильтр = "Файл настроек (*.conf)|*.conf";
	ДиалогСохраненияФайла.Заголовок = "Укажите имя сохранения";
	
	РезультатВыбора = Ждать ДиалогСохраненияФайла.ВыбратьАсинх();  
	...
	Для Каждого Секция Из СписокНастроек Цикл
		Т.ДобавитьСтроку(СтрШаблон("%1	%2", Секция.Ключ, Секция.Значение));	
	КонецЦикла;                                                            
	
	Т.Записать(ДиалогСохраненияФайла.ПолноеИмяФайла);
...
&НаСервере
Функция НастройкиБрокера()
	
	Результат = Новый Массив;
	
	Для Каждого Настройка Из Объект.ПараметрыПодключения Цикл
		Результат.Добавить(Новый Структура("Ключ, Значение", Настройка.Ключ, Настройка.Значение));	
	КонецЦикла;
