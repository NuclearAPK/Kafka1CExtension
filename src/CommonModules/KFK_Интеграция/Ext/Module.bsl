...
Функция ПодключитьКомпонентуКафка(ИмяКомпоненты, ТипПодключения = Неопределено) Экспорт
	...
	Попытка
		...
		//Подключено = ПодключитьВнешнююКомпоненту("C:\Sources\Projects\Simple-Kafka_Adapter\out\build\x64-Release\Release\SimpleKafka1C.dll",
...
Функция ПолучитьДанныеТопиковБрокера(АдресБрокера, ДополнительныеПараметры = Неопределено) Экспорт  
	...
	// проверяем доступность брокера, таймаут ожидания 10 сек. (по умолчанию)
	ТаймаутОжидания = Константы.KFK_ТаймаутОпросаБрокера.Получить();
	Если НЕ ЗначениеЗаполнено(ТаймаутОжидания) Тогда
		ТаймаутОжидания = 10;	
	КонецЕсли;               
	
	Если НЕ Компонента.ПроверитьДоступностьБрокера(АдресБрокера, ТаймаутОжидания * 1000) Тогда
		KFK_ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон("Брокер недоступен: %1", Компонента.ПолучитьСообщениеОбОшибке()));	
		Возврат ТопикиБрокера;
	КонецЕсли;
	...
	Если ТипЗнч(Данные) = Тип("Структура") И Данные.Свойство("topics") И ТипЗнч(Данные.topics) = Тип("Массив") Тогда
		Для каждого Топик Из Данные.topics Цикл
			Если Топик.Свойство("topic") Тогда       
				Если Топик.topic = "__consumer_offsets" Тогда
					Продолжить;
				КонецЕсли;
			...
			Для каждого СообщениеВКафку Из Сообщения Цикл			     
				...
				Для т = 1 по КоличествоСообщений Цикл
					...
					Если Синхронно Тогда          
						...
						КонецЕсли;
					Иначе                             
						РезультатОтправки = Компонента.ОтправитьСообщение(ДанныеСообщения, Топик, Партиция, Ключ, Заголовки);  
					КонецЕсли;   
				КонецЦикла;
			КонецЦикла;
		КонецЦикла;  
		
		// Остановка продюсера (обязательно!)
		Если Компонента.ОстановитьПродюсера() Тогда
			Сообщить("Продюсер успешно остановлен, все сообщения доставлены");
		Иначе
			// Не все сообщения были доставлены или произошла ошибка
			ОписаниеОшибки = Компонента.ПолучитьСообщениеОбОшибке();
			ЗаписьЖурналаРегистрации("Kafka.Producer", УровеньЖурналаРегистрации.Ошибка,,, "Ошибка остановки продюсера: " + ОписаниеОшибки);
		КонецЕсли;		
	Иначе       
		ТекстОшибки = "";  
